...
19. **Typed** — Every constant, property, parameter, and return type must be explicitly typed. The same when use the config() helper: `config()->integer('xxx.yyy')`, `config()->bool('xxx.yyy')`, etc.
