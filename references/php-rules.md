...
| Data DTOs | `InvoicePaidData`, `CreateInvoiceData` |
...
All API errors must return a single, predictable shape using RFC 9457 Problem+JSON (`application/problem+json`). Domain exceptions are not unexpected failures — they are known, named outcomes and deserve informative 4xx responses, not generic 500s.

Full implementation details — exception configuration in `bootstrap/app.php`, the `problem()` helper, `title_for_status()`, domain exception registration, status mapping, and test patterns — are in `references/api-patterns.md`.
